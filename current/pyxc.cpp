...
static int LogLiteralDecodeError(LiteralDecodeError Error,
  ...
  fprintf(stderr, "Error (Line %d, Column %d): %s in %s literal\n",
          CurrentTokenLocation.Line, CurrentTokenLocation.Column, Message, LiteralKind);
  PrintErrorSourceContext(CurrentTokenLocation);
  return tok_error;
}
...
static int getToken() {
  ...
  if (isdigit(LexerLastChar) || (LexerLastChar == '.' && isdigit(peek()))) {
    ...
    if (LexerLastChar == 'e' || LexerLastChar == 'E') {
      ...
      LogInvalidNumberLiteralAtLocation(NumStr, CurrentTokenLocation);
      return tok_error;
      }
      ConsumeDigits();
    }
...
public:
  FunctionSignatureNode(const string &Name, vector<ParameterInfo> Parameters,
      ...
      : Name(Name), Parameters(std::move(Parameters)), ReturnType(ReturnType),
        ReturnStructName(std::move(ReturnStructName)), IsVarArg(IsVarArg),
        Loc(Loc) {}
  ...
  std::unique_ptr<FunctionSignatureNode> clone() const {
    return std::make_unique<FunctionSignatureNode>(Name, Parameters, Loc, ReturnType,
                                          IsVarArg, ReturnStructName);
  }
...
static void BeginFunctionScope(const vector<FunctionSignatureNode::ParameterInfo> &Parameters) {
  ...
  for (const auto &Parameter : Parameters) {
    FunctionScope[Parameter.Name] = Parameter.Type;
    if (Parameter.Type == ValueType::Struct || Parameter.Type == ValueType::Pointer ||
        Parameter.Type == ValueType::Array)
      FunctionStructScope[Parameter.Name] = Parameter.StructName;
  }
}
...
unique_ptr<ExpressionNode> LogErrorExpression(const string &ErrorMessage) {
  HadError = true;
  SourceLocation Anchor = GetDiagnosticAnchorLoc(CurrentTokenLocation, CurrentToken);
...
static ValueType ParseTypeToken(string *StructName) {
  ...
  case tok_name: {
    ...
    if (!StructTypes.count(TyName)) {
      LogErrorExpression(("Unknown type '" + TyName + "'"));
      return ValueType::Error;
    }
    getNextToken();
...
static unique_ptr<ExpressionNode> ParseNameExpressionWithName(const string &ParsedName) {
    ...
    if (InitSignature) {
      ...
      if (Arguments.size() != ExpectedArgs)
        return LogErrorExpression(
            ...
            ParsedName + "': expected " + to_string(ExpectedArgs) +
  ...
  if ((!Signature->isVarArg() && Signature->getNumParameters() != Arguments.size()) ||
    ...
    return LogErrorExpression(
        ...
        (Signature->isVarArg() ? " or more, got " : ", got ") +
        to_string(Arguments.size()));

  for (size_t i = 0; i < Arguments.size() && i < Signature->getNumParameters(); ++i) {
    ValueType ArgType = Arguments[i]->getType();
...
static unique_ptr<FunctionDefinitionNode> ParseTopLevelExpression() {
  ...
  auto Signature = make_unique<FunctionSignatureNode>(
      FnName, vector<FunctionSignatureNode::ParameterInfo>(), CurrentTokenLocation, RetType);
...
static bool ParseAggregateDefinition(const char *KindName) {
  ...
  if (CurrentToken != tok_name) {
    LogErrorExpression((string("Expected ") + KindName + " name"));
    return false;
  }
  ...
  if (StructTypes.count(StructName)) {
    LogErrorExpression(("Aggregate '" + StructName + "' is already defined"));
    return false;
  }
  ...
  while (CurrentToken != tok_dedent && CurrentToken != tok_eof) {
    ...
    if (CurrentToken == tok_def) {
      ...
      if (CurrentToken == tok_eol)
        ...
        // The method body was itself an indented block; ParseSuite
    ...
    if (FieldType == ValueType::Error || FieldType == ValueType::None) {
      LogErrorExpression((string("Invalid ") + KindName + " field type"));
      return false;
    }
...
VerifyTraitConformance(const string &ClassName,
  ...
  if (CI == StructTypes.end() || !CI->second.IsClass) {
    LogErrorExpression(("Unknown class '" + ClassName + "'"));
    return false;
  }
  if (!Traits.count(TraitName)) {
    LogErrorExpression(("Unknown trait '" + TraitName + "'"));
  ...
  if (!TI.TypeParamName.empty() && !ImplRef.HasTypeArg) {
    LogErrorExpression(("Trait '" + TraitName + "' requires a type argument"));
    return false;
  }
  if (TI.TypeParamName.empty() && ImplRef.HasTypeArg) {
    LogErrorExpression(
...
static bool ParseTraitDefinition() {
  ...
  if (Traits.count(TraitName) || StructTypes.count(TraitName) ||
    ...
    LogErrorExpression(("Name '" + TraitName + "' is already defined"));
    return false;
  }
  getNextToken(); // eat trait name
  string TypeParamName;
  ...
  while (CurrentToken != tok_dedent && CurrentToken != tok_eof) {
    ...
    if (Duplicate) {
      LogErrorExpression(("Duplicate trait method '" + MethodName + "'"));
      return false;
    }
...
Function *FunctionSignatureNode::codegen() {
  ...
  for (auto &Argument : TheFunction->args())
    Argument.setName(Parameters[ParameterIndex++].Name);
