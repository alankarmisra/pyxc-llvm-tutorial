# 15. Blocks, elif, Optional else, and Benchmarking

If Chapter 14 gave us indentation tokens, Chapter 15 is where we finally cash that check.
...
> We finally fixed it in Chapter 24 when we revisited signed/unsigned and truth-value correctness.
...
## Build Setup (same idea as Chapter 14)

If you built Chapter 14, this should look familiar. Chapter 15 keeps the same Makefile shape and toolchain assumptions.

From `code/chapter15/Makefile`:
...
Chapter 14 gave us `indent`/`dedent`. Chapter 15 turns those into a suite AST.
...
Chapter 15 introduces mode-aware `main` handling:
...
- `code/chapter15/bench/run_suite.sh`
- `code/chapter15/bench/cases/*.py`
- `code/chapter15/bench/cases/*.pyxc`
...
```bash
code/chapter15/pyxc -i code/chapter15/test/blocks_bad_indent.pyxc
```
