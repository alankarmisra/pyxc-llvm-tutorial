# 16. Typed Interop: Core Types, Pointers, and ABI Correctness

Chapter 15 cleaned up operators and short-circuit behavior.  
In Chapter 16, we take the next major step: a practical type system that can interoperate with C APIs safely.
...
That is exactly what Chapter 16 introduces.
...
Chapter 16 uses Python-flavored pointer forms:
...
Chapter 16 adds key tokens and keywords:
...
Chapter 16 also introduces default C-like aliases initialized from target data layout:
...
Chapter 15 tracked mostly allocas. Chapter 16 stores richer per-variable metadata (alloca + type hints) so pointer operations and print/type dispatch can work correctly.
...
Chapter 16 applies extension attributes in prototype codegen:
...
`code/chapter16/runtime.c` now exports typed print/char helpers for scalar widths.
...
To reduce cascading noise in file mode, Chapter 16 adds a stop-after-first-semantic-error gate:
...
Chapter 16 test coverage lives under `code/chapter16/test/`.
...
Chapter 16 turns Pyxc from a mostly-untyped toy frontend into an interop-capable typed frontend:
...
With this foundation in place, Chapter 17 can add language-level printing ergonomics (`print(...)`) without re-solving basic type and ABI correctness.

## Full Source Code Listing

If you prefer reading the complete implementation inline while following this chapter, here are the full Chapter 16 sources.

You can also browse these files directly on GitHub:

- [Chapter 16 `pyxc.cpp`](https://github.com/alankarmisra/pyxc-llvm-tutorial/blob/main/code/chapter16/pyxc.cpp)
...
Try creating a few new `.pyxc` files under `code/chapter16/test/` that cover:
