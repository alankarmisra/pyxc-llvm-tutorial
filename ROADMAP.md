...
| # | Title | Notes |
|---|-------|-------|
| 96 | Real Source Locations in Codegen Diagnostics | Properly closes the stale-`CurLoc` known bug: `SourceLoc` field on `ExpressionNode`, propagated through every node constructor, so every codegen error — not just parse errors — points at the right line and column |
