...
`ModuleHasGlobals` is set by `VarStatementNode::codegen` when it emits a `GlobalVariable`. If it's set, I keep the module permanently — freeing it would destroy the global's storage. If not, the old ResourceTracker path from chapter 6 applies and the module is freed after execution.

The two branches print with different formats — `"%f\n"` when the module sticks around, `"Evaluated to %f\n"` when it gets freed. That's not a deliberate stylistic choice, it's just what falls out of keeping each branch's own `printf` call where chapter 6 originally put it. I'm noting it here because it's the kind of small inconsistency I'd otherwise forget I introduced, and a reader diffing REPL output against expectations deserves to know it's real, not a typo.
...
In [Chapter 14](chapter-14.md) I add object file emission. Instead of JIT-compiling everything at runtime, pyxc can write a `.o` file that a system linker combines with other objects into a standalone native binary — no JIT required.
