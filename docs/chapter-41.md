...
---
# 41. pyxc: Assignment as Expression

## What I Am Building

[Chapter 40](chapter-40.md) added unsigned integer types. pyxc can call `getchar()`, but the canonical K&R idiom for reading until EOF still doesn't compile:
...
`ParseExpression` already parses one full `logical-or` — the top of the operator-precedence chain from Chapter 35. I add a check after it returns: if an assignment operator follows, treat what I just parsed as an lvalue and parse the right-hand side by recursing into `ParseExpression` again:
...
[Chapter 42](chapter-42.md) adds variadic `extern` declarations, enabling `printf`, `scanf`, and other C variadic functions.
