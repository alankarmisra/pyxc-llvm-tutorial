...
| `TheJIT` | `JIT` |
| Repeated `"__anon_expr"` literal | `AnonymousExpressionFunctionName` |
...
**`cppdiff` markers** (a project-specific fence type — not literal `diff` output). Only three:
...
There is no fourth marker for "mostly new, don't bother diffing precisely." A block that used to reach for something softer than `+`/`-` (a `~` marker) is retired — always resolve down to real `+`/`-`/`*`, even when that means more `+`/`-` lines than an idealized diff would have.

**When the real source's own line-wrapping obscures a small change, reflow the block instead of reaching for a softer marker.** This comes up with long parameter lists and initializer lists: adding one parameter to a multi-line, multi-per-line constructor signature often makes every line's wrapping shift, so a byte-accurate diff against the real source ends up almost entirely `-`/`+` even though only one thing conceptually changed. In that case, rewrite the block with one entry per line (one parameter per line, one initializer per line) so the single real addition sits on its own `+` line surrounded by unchanged `*` lines — and say so explicitly right before the block ("reflowed one per line so the addition is easy to spot; the real source wraps this differently"). This is the one place a chapter's code block is allowed to diverge from the real file's exact line breaks. It must still be the same tokens in the same order — verify by stripping whitespace from both the reflowed block's `*`/`+` lines and the real source and diffing those; only formatting may differ, never content.
...
- [ ] Any bare/context-free `cppdiff` or `cpp` block follows §4.5: incremental changes are diffed against the real previous-chapter source with `+`/`-`/`*` markers (reflowed one-entry-per-line when the real wrapping would obscure a small change — see §4.5); brand-new code gets real, verified enclosing context instead of an invented wrapper
