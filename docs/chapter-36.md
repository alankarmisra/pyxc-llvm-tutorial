...
[Chapter 35](chapter-35.md) added bitwise operators. Right now, multi-way branching on an integer value means a chain of `if`/`elif`. I'm adding `switch`:
...
Chapter 34's `LoopControlStack` carries `BreakTarget` and `ContinueTarget` together. A switch needs to push a break target without disturbing `continue`, which still has to reach the enclosing loop — so I add a separate stack just for break:
...
Phase 5 — K&R compatibility — is done. I've now got `while`, `do/while`, `break`, `continue`, `switch`, the full set of arithmetic and bitwise operators, `++`/`--`, compound assignment, logical operators, and the C memory model from Chapters 17–22. Next up: modules, imports, and the standard library.
