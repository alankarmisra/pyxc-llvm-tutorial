# 13. From REPL-Only to a Real Compiler Pipeline
...
Chapter 13 still uses the pre-indentation grammar shape (newline-delimited statements).
...
This chapter ships with a ready-to-use build script in [`code/chapter13`](https://github.com/alankarmisra/pyxc-llvm-tutorial/tree/main/code/chapter13).
...
```bash
code/chapter13/build/pyxc -i code/chapter13/fib.pyxc
```

Run the chapter tests (when a test suite exists):

```bash
cd code/chapter13/test
lit -sv .
```
