...
---
# 13. Pyxc: Mutable Variables

## What We're Building

By chapter 12, we can write control flow (`if` / `while`), but we still can’t update local state.
...
Grab the code: [code/chapter13](https://github.com/alankarmisra/pyxc-llvm-tutorial/tree/main/code/chapter13)

Or clone the whole repo:

```bash
git clone https://github.com/alankarmisra/pyxc-llvm-tutorial
cd pyxc-llvm-tutorial/code/chapter13
```
...
This chapter includes **83 automated tests**. New mutable-focused tests include:
...
Control-flow and indentation behavior from chapter 10 (including `elif` and
tab-consistent indentation) remains covered by the inherited suite.
