# 25. Immutable Bindings with const

Chapter 25 introduces `const` local declarations with compile-time reassignment protection.
...
In `code/chapter25/pyxc.ebnf`, we add a dedicated statement production:
...
In `code/chapter25/pyxc.cpp`, add a token kind and keyword mapping.
...
Chapter 25 adds a separate AST node for const declaration and extends binding metadata.

`VarBinding` in `code/chapter25/pyxc.cpp` now has:
...
In assignment codegen, Chapter 25 adds:
...
Chapter 25 adds four focused tests in `code/chapter25/test`.
...
This gives us safer local semantics before we scale into multi-file workflows in Chapter 26.
