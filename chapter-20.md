# 20. Structs and Named Field Access

Chapter 19 gave us mature control flow and a stronger operator set.
...
Chapter 20 introduces that missing piece: `struct`.
...
That one construct creates a foundation for all later data features, including arrays-of-structs (Chapter 21), heap-allocated structs (Chapter 22), and pointer-oriented interop work.
...
Chapter 20 intentionally does *not* support:
...
In `code/chapter20/pyxc.ebnf`, Chapter 20 introduces `struct_decl` and member-expression forms.
...
Chapter 20 adds `struct` as a keyword token:
...
So Chapter 20 tightens the check: `.` starts a number **only if the next char is a digit**.
...
Chapter 20 adds a dedicated top-level parser path:
...
Chapter 20 extends that loop with member suffix handling:
...
Chapter 20 introduces internal metadata structures:
...
Chapter 20 extends type resolution so alias/base names can resolve to struct types.
...
## 9. Diagnostics added in Chapter 20
...
## 10. Tests added in Chapter 20

Chapter 20 preserves Chapter 19 tests and adds struct-focused tests:
...
Chapter 20 was also checked against Chapter 19 behavior as a regression sanity step.

## 12. Design takeaways

What Chapter 20 establishes:
...
- Chapter 21 uses structs + indexing together (`array[Point, N]`)
- Chapter 22 allocates structs on heap (`ptr[Point] = malloc[Point](...)`)

In short: Chapter 20 is where the language stops being scalar-centric.
