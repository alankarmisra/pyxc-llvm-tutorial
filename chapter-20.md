...
Chapter 19 introduced structs and named field access.

Chapter 20 adds fixed-size arrays as first-class types so we can model contiguous collections with explicit element type and length.

In this chapter we support:

- array type syntax in type positions: `array[T, N]`
- array indexing for loads/stores: `a[i]`
- arrays in structs and structs in arrays
- reuse of existing pointer indexing behavior
...
1. `TypeExpr` extended with an `Array` kind and compile-time size.
2. `ParseTypeExpr()` extended to parse `array[ElemType, N]`.
3. Type resolution lowered arrays to LLVM `ArrayType`.
4. Pointee/leaf-type helpers updated so array element types flow through existing code paths.
5. `IndexExprAST::codegenAddress()` now supports both pointer and array bases.
6. Struct/array initializer policy clarified (no direct initializer expressions yet).
...
In `code/chapter20/pyxc.ebnf`, `type_expr` now includes `array_type`:
...
Index syntax did not change (we reuse existing `[...]` expression grammar).

## Tests First: Array Coverage

Chapter 20 keeps all inherited tests from Chapter 19 and adds array-focused tests:
...
# RUN: %pyxc -i %s > %t 2>&1
# RUN: grep -q "Error (Line:" %t
# RUN: grep -q "Array index must be an integer type" %t
...
### `TypeExpr` gets array representation

Chapter 19 had builtin/alias/pointer kinds. Chapter 20 adds:

```cpp
enum class TypeExprKind { Builtin, AliasRef, Pointer, Array };
```

and stores size on the node:
...
- bracket/comma/closing bracket shape
- size token must be an integer literal
...
`ResolveTypeExpr(...)` now lowers array types directly:
...
This allows arrays to appear anywhere normal types are resolved (locals, fields, aliases, function signatures where applicable).

## Indexing: Pointer Base + Array Base

The biggest behavior change is in `IndexExprAST::codegenAddress()`.

### Existing pointer path (kept)

If base evaluates to pointer, we keep pointer indexing semantics.

### New array path

If base value type is an array type, we compute element address via two-index GEP:

```cpp
Value *Zero = ConstantInt::get(Type::getInt64Ty(*TheContext), 0);
Builder->CreateGEP(BaseValTy, BaseAddr, {Zero, IdxV}, "arr.idx.addr");
```

This is the standard LLVM pattern for indexing into an alloca’d array aggregate.

### Index type diagnostics

Index must be integer-like in both paths. Chapter 20 uses:

```cpp
"Array index must be an integer type"
```

## Hint Propagation Adjustments

To preserve existing print/type behavior with indexed results:

- `ResolvePointeeTypeExpr` handles arrays by returning element type.
- `ResolveBuiltinLeafName`/`ResolvePointeeBuiltinLeafName` recurse through arrays.
- `IndexExprAST::getValueTypeHint()` returns array element type when base is array.

These small changes let existing typing/printing machinery keep working for array-indexed expressions.

## Initialization Policy

Direct initializer expressions for struct/array-typed locals are still out of scope. Chapter 20 makes this explicit:

```cpp
"Struct/array variables do not support direct initializer expressions"
```

So array locals are declared first, then populated by index assignments.
...
- `code/chapter20`: `make`
- `code/chapter20`: `lit -sv test` (`28/28`)
- regression sanity: `code/chapter19`: `lit -sv test` (`22/22`)
...
### `code/chapter20/test/lit.cfg.py`
...
# RUN: %pyxc -i %s > %t 2>&1
# RUN: grep -q "Error (Line:" %t
# RUN: grep -q "Expected positive integer literal array size" %t
...
def main() -> i32:
    ...
    b.vals[1] = 8
    b.vals[2] = 13
    print(b.vals[0], b.vals[1], b.vals[2])
    return 0
...
### `code/chapter20/test/for_continue_step_path.pyxc`
...
### `code/chapter20/test/struct_alias_type.pyxc`
...
### `code/chapter20/test/struct_basic_fields.pyxc`
...
### `code/chapter20/test/struct_error_duplicate_field.pyxc`
...
### `code/chapter20/test/struct_error_duplicate_struct.pyxc`
...
### `code/chapter20/test/struct_error_nonstruct_member.pyxc`
...
### `code/chapter20/test/struct_error_unknown_field.pyxc`
...
### `code/chapter20/test/struct_nested_fields.pyxc`
