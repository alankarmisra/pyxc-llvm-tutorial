# 26. Separate Compilation and Multi-File Linking

Chapter 26 upgrades pyxc from single-file flow to translation-unit flow.
...
Chapter 26 is primarily a driver/linker architecture chapter (multi-file compile/link).
Language grammar is effectively unchanged from Chapter 25.
...
In `code/chapter26/pyxc.cpp`, positional input changed to a list:
...
Chapter 26 adds a helper to derive per-input object names:
...
Chapter 26 fixes this with explicit reset functions in `code/chapter26/pyxc.cpp`.
...
Chapter 26 also makes `EnsureDefaultTypeAliases()` safe when module layout is not yet fully populated.
...
Chapter 26 resolves runtime object relative to the pyxc binary path:
...
## New Chapter 26 tests

Added in `code/chapter26/test`:
...
## Main driver flow after Chapter 26
...
Chapter 26 is the bridge from language features to practical project-scale compilation.
