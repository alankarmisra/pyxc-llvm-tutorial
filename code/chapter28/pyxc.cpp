...
class MatchStmtAST : public StmtAST {
  ...
  std::vector<std::pair<std::unique_ptr<ExprAST>, std::unique_ptr<BlockSuiteAST>>>
      Cases;
  std::unique_ptr<BlockSuiteAST> DefaultCase;

public:
  MatchStmtAST(
      SourceLocation Loc, std::unique_ptr<ExprAST> MatchExpr,
...
static std::unique_ptr<StmtAST> ParseMatchStmt() {
  ...
  return std::make_unique<MatchStmtAST>(MatchLoc, std::move(MatchExpr),
                                        std::move(Cases),
...
Value *CallExprAST::codegen() {
  ...
  if (Callee == "scanf") {
    if (Args.empty())
      return LogError<Value *>("scanf requires a format string");
    std::string Fmt;
    if (!Args[0]->getStringLiteralValue(Fmt))
      return LogError<Value *>("scanf format must be a string literal");
    std::vector<char> Specs;
    for (size_t I = 0; I < Fmt.size(); ++I) {
      if (Fmt[I] != '%')
        continue;
      if (I + 1 >= Fmt.size())
        return LogError<Value *>("Unsupported scanf format specifier '%'");
      ++I;
      if (Fmt[I] == '%')
        continue;
      while (I < Fmt.size() &&
             (Fmt[I] == '*' || Fmt[I] == '-' || Fmt[I] == '+' ||
              Fmt[I] == ' ' || Fmt[I] == '#' || Fmt[I] == '0' ||
              Fmt[I] == '.' || isdigit(static_cast<unsigned char>(Fmt[I])) ||
              Fmt[I] == 'l' || Fmt[I] == 'h'))
        ++I;
      if (I >= Fmt.size())
        return LogError<Value *>("Unsupported scanf format specifier '%'");
    ...
    if (Specs.size() != Args.size() - 1)
      return LogError<Value *>("scanf format/argument count mismatch");
    ...
    for (size_t I = 0; I < Specs.size(); ++I) {
      ...
      if (Spec == 'd') {
        ...
        if (PointeeLeaf != "f32" && PointeeLeaf != "f64")
          return LogError<Value *>("scanf type mismatch for float format");
      } else if (Spec == 's' || Spec == 'c') {
        if (PointeeLeaf != "i8" && PointeeLeaf != "u8")
          return LogError<Value *>("scanf type mismatch for character format");
      }
    }
  }
  ...
  if (Callee == "fopen") {
    ...
    if (!CheckPointerArg(0, "open expects pointer path argument") ||
        !CheckIntegerArg(1, "open expects integer flags argument") ||
        !CheckIntegerArg(2, "open expects integer mode argument"))
      return nullptr;
  } else if (Callee == "creat") {
    if (!CheckPointerArg(0, "creat expects pointer path argument") ||
        !CheckIntegerArg(1, "creat expects integer mode argument"))
      return nullptr;
  } else if (Callee == "close") {
    if (!CheckIntegerArg(0, "close expects integer fd argument"))
      return nullptr;
  } else if (Callee == "read") {
    ...
    if (!CheckIntegerArg(0, "write expects integer fd argument") ||
        !CheckPointerArg(1, "write expects pointer buffer argument") ||
        !CheckIntegerArg(2, "write expects integer count argument"))
      return nullptr;
  } else if (Callee == "unlink") {
    if (!CheckPointerArg(0, "unlink expects pointer path argument"))
      return nullptr;
  } else if (Callee == "isdigit") {
    if (!CheckIntegerArg(0, "isdigit expects integer argument"))
      return nullptr;
  } else if (Callee == "isalpha") {
    if (!CheckIntegerArg(0, "isalpha expects integer argument"))
      return nullptr;
  } else if (Callee == "isspace") {
    if (!CheckIntegerArg(0, "isspace expects integer argument"))
      return nullptr;
  } else if (Callee == "tolower") {
    if (!CheckIntegerArg(0, "tolower expects integer argument"))
      return nullptr;
  } else if (Callee == "toupper") {
    if (!CheckIntegerArg(0, "toupper expects integer argument"))
      return nullptr;
  }
...
Value *MatchStmtAST::codegen() {
  ...
  for (size_t I = 0; I < Cases.size(); ++I) {
    ...
    BasicBlock *NextCheckBB =
        (I + 1 < Cases.size())
            ? BasicBlock::Create(*TheContext, "match.next")
            : DefaultBB;
...
Function *PrototypeAST::codegen() {
  ...
  if (!RetTy)
    return nullptr;
  bool IsMainEntry = UseCMainSignature && Name == "main";
  if (IsMainEntry)
...
static bool TypeExprEqual(const TypeExprPtr &L, const TypeExprPtr &R) {
  if (L == R)
    return true;
  if (!L || !R)
    return false;
...
static bool PrototypeCompatible(const PrototypeAST &A, const PrototypeAST &B) {
  if (A.getName() != B.getName())
    return false;
  if (A.getArgTypes().size() != B.getArgTypes().size())
    return false;
  for (size_t I = 0; I < A.getArgTypes().size(); ++I) {
    if (!TypeExprEqual(A.getArgTypes()[I], B.getArgTypes()[I]))
      return false;
  }
  return TypeExprEqual(A.getRetType(), B.getRetType());
}

static std::unique_ptr<PrototypeAST> ClonePrototype(const PrototypeAST &P) {
  SourceLocation Loc{P.getLine(), 0};
...
static bool RegisterPrototypeForLookup(const PrototypeAST &Proto) {
  ...
  if (Existing == FunctionProtos.end()) {
    FunctionProtos[Proto.getName()] = ClonePrototype(Proto);
    return true;
  }
  ...
  if (!PrototypeCompatible(*Existing->second, Proto)) {
    HadError = true;
    fprintf(stderr,
            "%sError (Line: %d, Column: 1): Function redeclared with "
            "incompatible signature: %s\n%s",
            Red, Proto.getLine(), Proto.getName().c_str(), Reset);
    return false;
  }
...
int main(int argc, char **argv) {
  ...
  if (InputFilenames.empty()) {
    ...
    case Interpret:
      ...
      InterpretFile(InputFilenames[0]);
      break;
