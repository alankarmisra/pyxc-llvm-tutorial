...
PY="$ROOT/bench/fib_bench.py"
PYXC_SRC="$ROOT/bench/fib_bench.pyxc"
PYXC_BIN="$ROOT/bench/fib_bench"
RUNS="${1:-3}"
...
echo "Compiling pyxc executable benchmark target..."
(
  cd "$ROOT"
  rm -f "$PYXC_BIN" fib_bench.o
...
py_out="$(python3 "$PY" | extract_last_float)"
pyxc_i_out="$("$ROOT/pyxc" -i "$PYXC_SRC" 2>&1 | extract_last_float)"
pyxc_x_out="$("$PYXC_BIN" 2>&1 | extract_last_float)"
