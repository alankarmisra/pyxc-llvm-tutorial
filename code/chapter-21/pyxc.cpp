...
static unique_ptr<ExpressionNode> ParseVariableStatement() {
  ...
  while (true) {
    ...
    VariableBindings.push_back({ParsedName, DeclType, std::move(Init)});
    if (IsGlobalDecl)
      GlobalVarTypes[ParsedName] = DeclType, GlobalVarDecls.insert(ParsedName);
    else
      DeclareVariable(ParsedName, DeclType);
...
static AllocaInst *CreateEntryBlockAlloca(Function *TheFunction,
  ...
  return TmpB.CreateAlloca(LLVMTypeFor(Type), nullptr, VariableName);
}
    ...
    for (auto &Var : VariableBindings) {
      ...
      if (!GV) {
        ...
        GV = new GlobalVariable(*TheModule, Type, false,
                                ...
                                ZeroConstant(VarType), VariableName);
