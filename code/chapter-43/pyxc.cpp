...
static map<int, string> TokenNames = [] {
  ...
  static map<int, string> Names = {
      ...
      {tok_trait, "'trait'"},
      {tok_impl, "'impl'"},
      {tok_module, "'module'"}, {tok_export, "'export'"},
...
static bool VerifyTraitConformance(const string &ClassName,
                                   const StructTypeInfo::TraitReference &Trait);
...
static bool ParseAggregateDefinition(const char *KindName) {
  ...
  vector<StructTypeInfo::TraitReference> ImplementedTraits;
  if (IsClass && CurrentToken == tok_lparen) {
    ...
    while (CurrentToken != tok_rparen) {
      ...
      StructTypeInfo::TraitReference TraitReference;
      TraitReference.Name = Name;
      string TraitName = TraitReference.Name;
      ...
      ImplementedTraits.push_back(std::move(TraitReference));
  ...
  for (const auto &Trait : StructTypes[AggregateName].ImplementedTraits) {
    if (!VerifyTraitConformance(AggregateName, Trait))
      return false;
  }
...
///   = "impl" trait-reference "for" name ":" end-of-lines
...
static void HandleImplementationDefinition() {
  bool Parsed = ParseImplementationDefinition();
  bool HasTrailing = CurrentToken != tok_eol && CurrentToken != tok_eof &&
                     CurrentToken != tok_block_end;
  if (!Parsed || HasTrailing) {
    if (Parsed)
