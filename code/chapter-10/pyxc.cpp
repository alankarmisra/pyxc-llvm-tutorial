...
static unique_ptr<FunctionSignatureNode> ParseFunctionSignature() {
  ...
  if ((FunctionName.size() == 7 && FunctionName.rfind("binary", 0) == 0 &&
       isascii(static_cast<unsigned char>(FunctionName[6])) &&
       ispunct(static_cast<unsigned char>(FunctionName[6]))) ||
      (FunctionName.size() == 6 && FunctionName.rfind("unary", 0) == 0 &&
       isascii(static_cast<unsigned char>(FunctionName[5])) &&
       ispunct(static_cast<unsigned char>(FunctionName[5])))) {
    fprintf(stderr,
...
///     ret-like value = 0.0
///
/// The loop variable name is rebound to `%i` while generating
/// Condition/Step/Body, then restored after the loop.
