...
static void LogErrorAtLocation(const string &ErrorMessage,
...
static void LogErrorAtLocation(const string &ErrorMessage,
  ...
  fprintf(stderr, "Error (Line %d, Column %d): %s\n", Location.Line,
          Location.Column, ErrorMessage.c_str());
...
public:
  AssignmentStatementNode(const string &VariableName,
                          unique_ptr<ExpressionNode> Expression)
...
public:
  ForStatementNode(const string &VariableName, bool DeclaresVariable,
...
public:
  IfStatementNode(unique_ptr<ExpressionNode> Condition,
...
struct FunctionScopeGuard {
  FunctionScopeGuard(const vector<string> &Parameters) {
...
static bool ParseForParts(unique_ptr<ExpressionNode> &Start,
                          unique_ptr<ExpressionNode> &Condition,
                          unique_ptr<ExpressionNode> &Update,
                          unique_ptr<ExpressionNode> &Body) {
...
///     ":" suite ;
/// The final expression performs the complete loop update; its value is
/// discarded. for-update = assignment-statement | expression ;
///
...
static unique_ptr<ExpressionNode> ParseForStatement() {
  ...
  // CurrentToken is tok_block_end (body was a block) or tok_eol (body was
  // inline). The enclosing ParseBlock loop handles both without any extra
  // boolean.
  return make_unique<ForStatementNode>(VariableName, DeclaresVariable,
                                       std::move(Start), std::move(Condition),
...
static unique_ptr<ExpressionNode> ParseIfStatement() {
  ...
  // If Then was an indented block, ParseBlock left tok_block_end in
  // CurrentToken. Consume it so we can look for 'else'; if there is no else we
  // re-inject it so the enclosing ParseBlock loop gets the signal.
...
// Parse a name-led expression, then recognize an assignment statement if '='
// follows.
static unique_ptr<ExpressionNode> ParseLeadingNameSimpleStatement() {
  auto Expr = ParseExpression();
  if (!Expr)
...
static unique_ptr<ExpressionNode> ParseBlock() {
  ...
  // as CurrentToken on return, removing the need for any BlockJustEnded
  // boolean.
...
/// AssignmentStatementNode::codegen - Evaluate the Right, store it into the
/// variable's stack slot, and produce the assigned value.
...
Value *ForStatementNode::codegen() {
  ...
  if (!Body->codegen())
    return nullptr;
  // BlockStatementNode restores NamedValues when the body finishes, but the
  // loop variable's alloca remains valid. We use the alloca directly for the
  // step.
...
/// VariableStatementNode::codegen - Allocate mutable local variables and
/// initialize them.
