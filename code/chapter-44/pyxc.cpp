...
static bool IsBitwiseOp(int Operator) { return Operator == '&' || Operator == '|' || Operator == '^'; }
...
static ValueType GetBinaryResultType(int Operator, ValueType L, const string &LStruct,
  ...
  if (IsArithmeticOp(Operator)) {
    ...
    if (Operator == '%' && (!IsIntType(L) || !IsIntType(R)))
      return ValueType::Error;
...
/// expression
///   = lvalue assignop expression | unaryexpr binary-operator-right ;
static unique_ptr<ExpressionNode> ParseExpression() {
  auto Left = ParseUnary();
  if (!Left)
    return nullptr;
  Left = ParseBinaryOperatorRight(0, std::move(Left));
  if (!Left)
    return nullptr;

  if (CurrentToken != '=' && !IsCompoundAssignTok(CurrentToken))
    return Left;
...
static bool ParseDottedModuleName(string &OutName) {
  ...
  while (CurrentToken == '.') {
    getNextToken(); // eat '.'
    if (CurrentToken != tok_name) {
      LogError("Expected name after '.' in module path");
      return false;
    }
    OutName += ".";
    OutName += Name;
...
static Value *EmitBuiltInArithmetic(int Operator, Value *L, ValueType LType,
  ...
  if (Operator == '-' && ResultType == ValueType::Int64 &&
