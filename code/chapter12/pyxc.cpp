...
static std::vector<std::string> IndentStack = {""};
  ...
  while (true) {
    ...
    if (AtLineStart) {
      ...
      while (LastChar == ' ' || LastChar == '\t') {
        IndentPrefix.push_back(static_cast<char>(LastChar));
        LastChar = advance();
      }
      ...
      const std::string &CurrentIndent = IndentStack.back();
      if (IndentPrefix.size() > CurrentIndent.size() &&
          IndentPrefix.rfind(CurrentIndent, 0) == 0) {
        const std::string Delta = IndentPrefix.substr(CurrentIndent.size());
        const bool HasSpaces = Delta.find(' ') != std::string::npos;
        const bool HasTabs = Delta.find('\t') != std::string::npos;
        if (HasSpaces && HasTabs) {
          HadError = true;
          CurLoc = SourceLocation{LexLoc.Line,
                                  static_cast<int>(IndentPrefix.size()) + 1};
          fprintf(stderr,
        ...
        IndentStack.push_back(IndentPrefix);
        PendingTokens.push_back(
            {tok_indent,
             SourceLocation{LexLoc.Line, static_cast<int>(IndentPrefix.size()) + 1}});
        AtLineStart = false;
        continue;
      }
      ...
      if (IndentPrefix.size() < CurrentIndent.size()) {
        while (IndentStack.size() > 1 &&
               IndentPrefix.size() < IndentStack.back().size()) {
          IndentStack.pop_back();
          PendingTokens.push_back(
              {tok_dedent,
               SourceLocation{LexLoc.Line, static_cast<int>(IndentPrefix.size()) + 1}});
        ...
        if (IndentStack.back() != IndentPrefix) {
          HadError = true;
          CurLoc = SourceLocation{LexLoc.Line,
                                  static_cast<int>(IndentPrefix.size()) + 1};
          fprintf(stderr,
...
static std::unique_ptr<ExprAST> ParseIfElseBranch();
...
static std::unique_ptr<ExprAST> ParseIfExpr() {
  ...
  auto ElseExpr = ParseIfElseBranch();
  if (!ElseExpr)
    return nullptr;
...
/// Parse the trailing branch of an if-expression:
///   ::= 'elif' expression ':' suite (if-else-branch)
///   ::= 'else' ':' suite
static std::unique_ptr<ExprAST> ParseIfElseBranch() {
  if (CurTok == tok_elif) {
    getNextToken(); // consume 'elif'
    ...
    if (CurTok != ':')
      return LogError<ExprPtr>("Expected ':' after elif condition");
    getNextToken(); // consume ':'

    auto ElifThen = ParseIndentedExprSuite("elif");
    if (!ElifThen)
      return nullptr;

    auto ElifElse = ParseIfElseBranch();
    if (!ElifElse)
      return nullptr;

    return std::make_unique<IfExprAST>(std::move(ElifCond),
                                       std::move(ElifThen),
                                       std::move(ElifElse));
  }
  ...
  return ParseIndentedExprSuite("else");
}
