...
static map<string, Token> Keywords = {
    ...
    {"int32", tok_int32},     {"int64", tok_int64},     {"uint8", tok_uint8},
    {"uint16", tok_uint16},   {"uint32", tok_uint32},   {"uint64", tok_uint64},
    {"float", tok_float},
    ...
    {"private", tok_private}, {"ptr", tok_ptr},         {"addr", tok_addr},
    {"sizeof", tok_sizeof},   {"type", tok_type},       {"trait", tok_trait},
    {"impl", tok_impl},       {"module", tok_module},   {"import", tok_import},
    {"export", tok_export}};
...
static map<int, string> TokenNames = [] {
  ...
  static map<int, string> Names = {
      ...
      {tok_module, "'module'"},   {tok_import, "'import'"},
      {tok_export, "'export'"},
...
static int gettok() {
  ...
  if (LexerLastChar == '\'') {
    ...
    if (LexerLastChar == '\'' || LexerLastChar == '\n' || LexerLastChar == EOF) {
      fprintf(stderr,
              "Error (Line %d, Column %d): empty character literal\n",
              CurLoc.Line, CurLoc.Col);
      PrintErrorSourceContext(CurLoc);
      return tok_error;
    }
    ...
    if (LexerLastChar == '\\') {
      ...
      case '0':
        ...
        break;
      default:
        fprintf(stderr, "Error (Line %d, Column %d): invalid character escape\n",
                CurLoc.Line, CurLoc.Col);
        PrintErrorSourceContext(CurLoc);
        return tok_error;
      }
  ...
  if (LexerLastChar == '*') {
    int Tok = (peek() == '=') ? (advance(), tok_muleq) : '*';
    LexerLastChar = advance();
    return Tok;
  }

  if (LexerLastChar == '/') {
    int Tok = (peek() == '=') ? (advance(), tok_diveq) : '/';
    LexerLastChar = advance();
    return Tok;
  }

  if (LexerLastChar == '%') {
    int Tok = (peek() == '=') ? (advance(), tok_modeq) : '%';
    LexerLastChar = advance();
    return Tok;
  }
...
public:
  CompoundAssignmentExprAST(const string &Name, int Op, unique_ptr<ExprAST> RHS,
                            ValueType Type, const string &StructName = "")
      : Name(Name), Op(Op), RHS(std::move(RHS)) {
    setType(Type, StructName);
  }
  bool shouldPrintValue() const override { return false; }
  Value *codegen() override;
...
class FieldCompoundAssignmentExprAST : public ExprAST {
  unique_ptr<FieldExprAST> LHS;
  int Op;
  unique_ptr<ExprAST> RHS;

public:
  FieldCompoundAssignmentExprAST(unique_ptr<FieldExprAST> LHS, int Op,
...
public:
  IndexExprAST(string BaseName, vector<string> FieldPath, unique_ptr<ExprAST> Index,
               ValueType ElemType, const string &ElemStructName = "")
      : BaseName(std::move(BaseName)), FieldPath(std::move(FieldPath)),
...
class IndexCompoundAssignmentExprAST : public ExprAST {
  unique_ptr<IndexExprAST> LHS;
  int Op;
  unique_ptr<ExprAST> RHS;

public:
  IndexCompoundAssignmentExprAST(unique_ptr<IndexExprAST> LHS, int Op,
                                 unique_ptr<ExprAST> RHS, ValueType Type,
                                 const string &StructName = "")
      : LHS(std::move(LHS)), Op(Op), RHS(std::move(RHS)) {
    setType(Type, StructName);
...
public:
  IndexedFieldExprAST(unique_ptr<IndexExprAST> BaseIndex, vector<string> FieldPath,
                      ValueType Type, const string &StructName = "")
...
class IndexedFieldCompoundAssignmentExprAST : public ExprAST {
  unique_ptr<IndexedFieldExprAST> LHS;
  int Op;
  unique_ptr<ExprAST> RHS;

public:
  IndexedFieldCompoundAssignmentExprAST(unique_ptr<IndexedFieldExprAST> LHS,
                                        int Op, unique_ptr<ExprAST> RHS,
                                        ValueType Type,
                                        const string &StructName = "")
      : LHS(std::move(LHS)), Op(Op), RHS(std::move(RHS)) {
    setType(Type, StructName);
...
/// ConstructorCallExprAST - Expression class for class construction:
///   Point(1, 2)
...
public:
  ConstructorCallExprAST(const string &ClassName, vector<unique_ptr<ExprAST>> Args)
...
public:
  PrototypeAST(const string &Name, vector<ArgInfo> Args,
      ...
      : Name(Name), Args(std::move(Args)), ReturnType(ReturnType),
        ReturnStructName(std::move(ReturnStructName)), IsOperator(IsOperator), IsVarArg(IsVarArg),
...
static bool VerifyTraitConformance(
    const string &ClassName, const StructTypeInfo::ImplTraitRef &ImplRef);
...
static unique_ptr<ExprAST> ParseAddrExpr() {
  ...
  while (CurTok == '.') {
    ...
    if (!CanAccessClassMember(CurStruct, FD.IsPublic))
      return LogError(("Field '" + Field + "' is private on '" + CurStruct + "'")
                          .c_str());
    CurType = FD.Type;
...
static unique_ptr<ExprAST> ParseIdentifierExprWithName(string IdName) {
  if (CurTok != '(') { // Simple variable ref.
    ...
    return make_unique<VariableExprAST>(IdName, Type, LookupVarStructName(IdName));
  }
  ...
  if (SI != StructTypes.end() && SI->second.IsClass) {
    ...
    if (InitProto) {
      ...
      if (MI != SI->second.MethodIsPublic.end() &&
        ...
        return LogError(("Method '__init__' is private on '" + IdName + "'")
                            .c_str());
      }
    }
    vector<unique_ptr<ExprAST>> Args;
...
static unique_ptr<ExprAST> ParseMethodCallExpr(unique_ptr<ExprAST> Receiver,
  ...
  if (MI != CI->second.MethodIsPublic.end() &&
    ...
    return LogError(("Method '" + MethodName + "' is private on '" + ClassName +
                     "'")
                        .c_str());
  }
  ...
  if (!Proto)
    return LogError(("Unknown method '" + MethodName + "' on '" + ClassName + "'")
                        .c_str());
  ...
  if (auto *Var = dynamic_cast<VariableExprAST *>(Receiver.get())) {
    ...
    // FieldExprAST always models an lvalue rooted at a base variable name.
...
static unique_ptr<FieldExprAST> ParseFieldAccessExpr(string BaseName,
  ...
  while (CurTok == '.') {
    ...
    const auto &FD = SI->second.Fields[FI->second];
    if (!CanAccessClassMember(CurStruct, FD.IsPublic)) {
      LogError(("Field '" + Field + "' is private on '" + CurStruct + "'").c_str());
      return nullptr;
    }
    CurType = FD.Type;
    CurStruct = FD.StructName;
  ...
  return make_unique<FieldExprAST>(std::move(BaseName), std::move(Path), CurType,
                                   CurStruct);
}
...
ParseIndexedFieldAccessExpr(unique_ptr<IndexExprAST> BaseIndex) {
  ...
  while (CurTok == '.') {
    ...
    auto FI = SI->second.FieldIndex.find(Field);
    if (FI == SI->second.FieldIndex.end())
      return LogError(("Unknown field '" + Field + "' on struct '" + CurStruct +
                       "'")
                          .c_str());
    const auto &FD = SI->second.Fields[FI->second];
    CurType = FD.Type;
...
static unique_ptr<ExprAST> ParseIdentifierExpr() {
  ...
  if (CurTok == '[') {
    if (auto *Var = dynamic_cast<VariableExprAST *>(Base.get())) {
      Base = ParseIndexExpr(Var->getName(), {}, Var->getType(), Var->getStructName());
...
static unique_ptr<ExprAST> ParseVarStmt() {
  ...
  while (true) {
    ...
    if (CurTok == '=') {
      ...
      if (!Init)
        return nullptr;
      bool ExactArrayInit = (DeclType == ValueType::Array &&
                             Init->getType() == ValueType::Array &&
                             DeclStructName == Init->getStructName() &&
                             dynamic_cast<ArrayLiteralExprAST *>(Init.get()) !=
                                 nullptr);
...
static unique_ptr<ExprAST> ParseExpression() {
  ...
  auto RHS = ParseExpression(); // right-associative assignment
  if (!RHS)
    return nullptr;
...
static unique_ptr<ExprAST> ParseCompoundAssignmentRHS(const string &Name,
  ...
  int BinOp = CompoundAssignToBinaryOp(AssignTok);
  if (!BinOp)
  ...
  ValueType ResultType =
      GetBinaryResultType(BinOp, DestType, DestStruct, RHS->getType(),
  ...
  if (DestType == ValueType::Pointer && DestStruct != ResultStructName)
    return LogError("Type mismatch in assignment");
  return make_unique<CompoundAssignmentExprAST>(Name, BinOp, std::move(RHS),
                                                DestType, DestStruct);
}
...
static unique_ptr<ExprAST>
ParseFieldCompoundAssignmentRHS(unique_ptr<FieldExprAST> LHS, int AssignTok) {
  ValueType DestType = LHS->getType();
  string DestStruct = LHS->getStructName();
  int BinOp = CompoundAssignToBinaryOp(AssignTok);
  if (!BinOp)
    return LogError("Unknown compound assignment operator");
  getNextToken(); // eat compound assignment token
  ...
  if (DestType == ValueType::Pointer && DestStruct != ResultStructName)
    return LogError("Type mismatch in assignment");
  return make_unique<FieldCompoundAssignmentExprAST>(
      std::move(LHS), BinOp, std::move(RHS), DestType, DestStruct);
}
...
static unique_ptr<ExprAST> BuildAssignmentExpr(int AssignTok,
  ...
  if (auto *Var = dynamic_cast<VariableExprAST *>(LHS.get())) {
    ...
    if (AssignTok == '=') {
      ...
      if ((DestType == ValueType::Pointer || DestType == ValueType::Array ||
          ...
          DestStruct != RHS->getStructName())
        return LogError("Type mismatch in assignment");
      return make_unique<AssignmentExprAST>(Var->getName(), std::move(RHS),
                                            DestType);
    }
    int BinOp = CompoundAssignToBinaryOp(AssignTok);
    if (!BinOp)
    ...
    ValueType ResultType = GetBinaryResultType(
        BinOp, DestType, DestStruct, RHS->getType(), RHS->getStructName(),
        &ResultStructName);
    if (ResultType == ValueType::Error || !IsAssignable(DestType, ResultType) ||
        (DestType == ValueType::Pointer && DestStruct != ResultStructName))
      return LogError("Type mismatch in assignment");
    return make_unique<CompoundAssignmentExprAST>(
        Var->getName(), BinOp, std::move(RHS), DestType, DestStruct);
  }
  ...
  if (auto *Field = dynamic_cast<FieldExprAST *>(LHS.get())) {
    ...
    if (AssignTok == '=') {
      ...
      if ((DestType == ValueType::Pointer || DestType == ValueType::Array ||
           DestType == ValueType::Struct) &&
          Owned->getStructName() != RHS->getStructName())
        return LogError("Type mismatch in assignment");
      return make_unique<FieldAssignmentExprAST>(std::move(Owned),
                                                 std::move(RHS), DestType);
    }
    int BinOp = CompoundAssignToBinaryOp(AssignTok);
    if (!BinOp)
      return LogError("Unknown compound assignment operator");
    ...
    if (ResultType == ValueType::Error || !IsAssignable(DestType, ResultType) ||
      ...
      return LogError("Type mismatch in assignment");
    return make_unique<FieldCompoundAssignmentExprAST>(
        std::move(Owned), BinOp, std::move(RHS), DestType, DestStruct);
  }
  ...
  if (auto *Idx = dynamic_cast<IndexExprAST *>(LHS.get())) {
    ...
    LHS.release();
    if (AssignTok == '=') {
      if (!IsAssignable(Owned->getType(), RHS->getType()))
        return LogError("Type mismatch in assignment");
      ...
      return std::make_unique<IndexAssignmentExprAST>(std::move(Owned),
                                                      std::move(RHS), T, S);
    }
    int BinOp = CompoundAssignToBinaryOp(AssignTok);
    if (!BinOp)
    ...
    string ResultStructName;
    ValueType ResultType =
        GetBinaryResultType(BinOp, Owned->getType(), Owned->getStructName(),
                            RHS->getType(), RHS->getStructName(),
                            &ResultStructName);
    if (ResultType == ValueType::Error ||
    ...
    ValueType T = Owned->getType();
    string S = Owned->getStructName();
    return std::make_unique<IndexCompoundAssignmentExprAST>(
        std::move(Owned), BinOp, std::move(RHS), T, S);
  }
  ...
  if (auto *IdxField = dynamic_cast<IndexedFieldExprAST *>(LHS.get())) {
    ...
    if (AssignTok == '=') {
      ...
      return std::make_unique<IndexedFieldAssignmentExprAST>(
          std::move(Owned), std::move(RHS), T, S);
    }
    int BinOp = CompoundAssignToBinaryOp(AssignTok);
    if (!BinOp)
      return LogError("Unknown compound assignment operator");
    ...
    ValueType T = Owned->getType();
    string S = Owned->getStructName();
    return std::make_unique<IndexedFieldCompoundAssignmentExprAST>(
        std::move(Owned), BinOp, std::move(RHS), T, S);
  }
...
static unique_ptr<ExprAST> ParseSimpleStmt() {
  ...
  if (CurTok == tok_identifier) {
    ...
    if (auto *Field = dynamic_cast<FieldExprAST *>(Expr.get())) {
      ...
      return ParseFieldCompoundAssignmentRHS(std::move(Owned), CurTok);
    }
    if (auto *Idx = dynamic_cast<IndexExprAST *>(Expr.get())) {
      int AssignTok = CurTok;
      getNextToken(); // eat '='
      ...
      if (AssignTok == '=') {
        ...
        ValueType ResultType = GetBinaryResultType(
            ...
            Idx->getStructName(), RHS->getType(), RHS->getStructName(),
            &ResultStructName);
        if (ResultType == ValueType::Error ||
      ...
      return std::make_unique<IndexCompoundAssignmentExprAST>(
          std::move(Owned), CompoundAssignToBinaryOp(AssignTok), std::move(RHS),
    ...
    if (auto *IdxField = dynamic_cast<IndexedFieldExprAST *>(Expr.get())) {
      ...
      if (AssignTok == '=') {
        ...
        ValueType ResultType = GetBinaryResultType(
            ...
            IdxField->getStructName(), RHS->getType(), RHS->getStructName(),
            &ResultStructName);
        if (ResultType == ValueType::Error ||
...
static ValueType ParseOptionalReturnTypeWithStruct(string &StructName,
...
static unique_ptr<FunctionAST> ParseMethodDefinitionInClass(
    const string &ClassName, bool IsPublic) {
  // CurTok is 'def'
  getNextToken(); // eat 'def'
  if (CurTok != tok_identifier)
  ...
  ArgNames.push_back(
      {"self", ValueType::Pointer, EncodePointerType(ValueType::Struct, ClassName)});
...
static bool ParseAggregateDefinition(const char *KindName) {
    ...
    if (CurTok != ')') {
      while (true) {
        ...
        if (SeenTraits.count(TraitName)) {
          LogError(("Duplicate trait '" + TraitName + "' in class implements list")
                       .c_str());
          return false;
        }
        ...
        if (!TI.TypeParamName.empty()) {
          if (CurTok != '[') {
            LogError(("Trait '" + TraitName + "' requires a type argument")
                         .c_str());
            return false;
          }
          getNextToken(); // eat '['
  ...
  while (CurTok != tok_dedent && CurTok != tok_eof) {
    ...
    if (CurTok != tok_identifier) {
      LogError((string("Expected field name in ") + KindName + " body").c_str());
      return false;
    }
...
static bool ParseDefinitionSignatureOnly() {
  ...
  FunctionProtos[Proto->getName()] = Proto->clone();
  if (CurTok != ':') {
    LogError("Expected ':' in definition");
    return false;
  }
  getNextToken(); // eat ':'
  SkipSignatureBody();
...
static bool ParseMethodSignatureOnlyInClass(const string &ClassName,
  ...
  if (CurTok != ')') {
    while (true) {
      if (CurTok != tok_identifier)
        return LogError("Expected parameter name in method prototype"), false;
      string ArgName = IdentifierStr;
      if (ArgName == "self")
        return LogError("Method parameters cannot be named 'self'"), false;
      getNextToken(); // eat identifier
      if (CurTok != ':')
        return LogError("Method parameters require a type annotation"), false;
      getNextToken(); // eat ':'
      string ArgStructName;
      ValueType ArgType = ParseTypeToken(&ArgStructName);
      if (ArgType == ValueType::Error)
        return false;
      if (ArgType == ValueType::None)
        return LogError("Parameters cannot have None type"), false;
      ArgNames.push_back({ArgName, ArgType, ArgStructName});
      if (CurTok == ')')
        break;
      if (CurTok != ',')
        return LogError("Expected ')' or ',' in parameter list"), false;
      getNextToken(); // eat ','
    }
  }
  ...
  if (RetType == ValueType::Error)
    return false;
  if (MethodName == "__init__" && RetType != ValueType::None)
    return LogError("Constructor '__init__' must return None"), false;

  string MangledName = ClassName + "." + MethodName;
  if (FunctionProtos.count(MangledName))
    return LogError(("Method '" + MethodName + "' is already defined on '" +
                     ClassName + "'")
                        .c_str()),
           false;
  ...
  if (CurTok != ':')
    return LogError("Expected ':' in function definition"), false;
  getNextToken(); // eat ':'
  SkipSignatureBody();
...
static bool ParseExportSignatureOnly() {
  ...
  LogError("Invalid export target");
  return false;
}
  ...
  while (CurTok != tok_dedent && CurTok != tok_eof) {
    ...
    if (CurTok != ')') {
      while (true) {
        ...
        Args.push_back({ArgName, ArgType, ArgStructName});
        if (CurTok == ')')
          break;
        if (CurTok != ',') {
...
static bool ParseImplDefinition() {
  ...
  if (!CI->second.IsClass) {
    LogError(("'" + ClassName +
              "' is a struct, not a class; traits can only be implemented on classes")
                 .c_str());
    return false;
  }
  ...
  auto SameImpl = [&](const StructTypeInfo::ImplTraitRef &R) {
    return R.TraitName == ImplRef.TraitName && R.HasTypeArg == ImplRef.HasTypeArg &&
           R.TypeArg == ImplRef.TypeArg &&
...
static Value *EmitCast(Value *V, ValueType From, ValueType To) {
  ...
  if (IsIntType(From) && IsIntType(To)) {
    ...
    return IsUnsignedIntType(From) ? Builder->CreateZExt(V, LLVMTypeFor(To), "zext")
...
Value *VariableExprAST::codegen() {
  ...
  if (auto *GV = GetGlobalVariable(Name))
    return (getType() == ValueType::Array)
               ? DecayArray(GV)
               : Builder->CreateLoad(LLVMTypeFor(getType(), getStructName()), GV,
                                     Name.c_str());
...
static Value *LoadPointerValue(const string &BaseName,
                               ...
                               ValueType BaseType, const string &BaseStructName);
...
Value *FieldExprAST::codegen() {
  ...
  string LeafStruct;
  Value *Ptr = GetFieldAddress(*getLValueName(), FieldPath, &LeafType, &LeafStruct);
  if (!Ptr)
...
static Value *BuildIndexElementPtr(IndexExprAST *IdxExpr) {
  ...
  if (IdxExpr->getFieldPath().empty()) {
    ...
    if (It != NamedValues.end() && It->second) {
      ...
      if (PtrType == ValueType::Pointer) {
        BasePtr =
            Builder->CreateLoad(LLVMTypeFor(ValueType::Pointer), It->second, "ptrload");
...
static Value *EmitBuiltInArithmetic(int Op, Value *L, ValueType LType,
  ...
  if (Op == '-' && ResultType == ValueType::Int64 &&
...
Value *IndexCompoundAssignmentExprAST::codegen() {
  Value *ElemPtr = BuildIndexElementPtr(LHS.get());
  if (!ElemPtr)
    return nullptr;
  ...
  if (!R)
    return nullptr;
  Value *Combined = EmitBuiltInArithmetic(Op, L, getType(), getStructName(), R,
                                          RHS->getType(), RHS->getStructName(),
...
Value *IndexedFieldCompoundAssignmentExprAST::codegen() {
  Value *BaseElemPtr = BuildIndexElementPtr(LHS->getBaseIndex());
  if (!BaseElemPtr)
    return nullptr;
  Value *Ptr = BaseElemPtr;
...
Value *CompoundAssignmentExprAST::codegen() {
  Value *R = RHS->codegen();
  if (!R)
    return nullptr;
  ...
  if (It != NamedValues.end() && It->second) {
    Value *L = Builder->CreateLoad(LLVMTypeFor(getType(), getStructName()),
                                   It->second, Name);
    Value *Combined = EmitBuiltInArithmetic(Op, L, getType(), getStructName(),
                                            R, RHS->getType(),
                                            RHS->getStructName(), getType(),
                                            getStructName());
    if (!Combined)
      return nullptr;
  ...
  if (auto *GV = GetGlobalVariable(Name)) {
    Value *L = Builder->CreateLoad(LLVMTypeFor(getType(), getStructName()), GV,
                                   Name);
    Value *Combined = EmitBuiltInArithmetic(Op, L, getType(), getStructName(),
                                            R, RHS->getType(),
                                            RHS->getStructName(), getType(),
...
Value *FieldAssignmentExprAST::codegen() {
  ...
  string DestStruct;
  Value *Ptr =
      GetFieldAddress(*LHS->getLValueName(), LHS->getFieldPath(), &DestType, &DestStruct);
  if (!Ptr)
    return LogErrorV("Unknown field access");
  Value *Val = RHS->codegen();
  if (!Val)
...
Value *FieldCompoundAssignmentExprAST::codegen() {
  ValueType DestType = ValueType::Error;
  string DestStruct;
  Value *Ptr =
      GetFieldAddress(*LHS->getLValueName(), LHS->getFieldPath(), &DestType, &DestStruct);
  if (!Ptr)
    return LogErrorV("Unknown field access");
  Value *L = Builder->CreateLoad(LLVMTypeFor(DestType, DestStruct), Ptr,
                                 "fieldload");
  Value *R = RHS->codegen();
  if (!R)
    return nullptr;
  Value *Combined = EmitBuiltInArithmetic(Op, L, DestType, DestStruct, R,
                                          RHS->getType(), RHS->getStructName(),
                                          getType(), getStructName());
  if (!Combined)
    return nullptr;
...
Value *BinaryExprAST::codegen() {
  if (Op == tok_and || Op == tok_or) {
    ...
    PHINode *PN = Builder->CreatePHI(Type::getInt1Ty(*TheContext), 2, "logictmp");
    if (Op == tok_and) {
  ...
  case '/':
  case '%': {
    return EmitBuiltInArithmetic(Op, L, LType, LHS->getStructName(), R, RType,
                                 RHS->getStructName(), getType(),
                                 getStructName());
  }
  case '&':
...
Value *ConstructorCallExprAST::codegen() {
  ...
  Function *CurFn = Builder->GetInsertBlock() ? Builder->GetInsertBlock()->getParent()
                                               : nullptr;
  if (!CurFn)
...
Value *SwitchExprAST::codegen() {
  ...
  BasicBlock *DefaultBB = DefaultCase
                              ? BasicBlock::Create(*TheContext, "switch.default", F)
                              : AfterBB;
  ...
  for (const auto &C : Cases) {
    ...
    auto *CaseConst = ConstantInt::get(
        cast<IntegerType>(CondLLVMType), static_cast<uint64_t>(C.first),
        /*isSigned=*/true);
...
static bool ResolveImportToPath(const string &ImporterPath, const string &Import,
                                string &OutPath) {
  ...
  while (CurTok != tok_eof) {
    ...
    if (CurTok == tok_import) {
      ...
      if (!ResolveImportToPath(CanonPath, ImportName, ImportPath)) {
        LogError(("Could not resolve import '" + ImportName + "' from '" +
                  CanonPath + "'")
                     .c_str());
        OK = false;
        break;
...
static bool PreloadImportedSignatures(const string &Path) {
  ...
  for (const auto &ImportName : ExtractTopLevelImports(Path)) {
    ...
    if (!ResolveImportToPath(Path, ImportName, ImportPath)) {
      LogError(("Could not resolve import '" + ImportName + "' from '" + Path +
                "'")
                   .c_str());
      return false;
    }
...
static bool LinkExecutable(const vector<string> &Inputs,
  ...
  auto TM = CreateTargetMachine();
  Triple TT = TM ? TM->getTargetTriple() : Triple(sys::getDefaultTargetTriple());
