# 28. Python-Style match/case

Chapter 28 adds `match/case` in a Python-style form.
...
The grammar update in `code/chapter28/pyxc.ebnf` introduces two non-terminals:
...
In `code/chapter28/pyxc.cpp`, the lexer/token layer adds:
...
New tests in `code/chapter28/test`:
...
```bash
code/chapter28/pyxc -i code/chapter28/test/c27_match_basic_default.pyxc
```
